...
class AuthToken:
    ...
    async def create_token(cls, data: User, expires_delta: Optional[float] = None) -> str:
        ...
        token: Annotated[str, Depends(AuthToken.oauth2_scheme)] = jwt_2.encode(claims=to_encode,
    ...
    async def verify_auth2(cls, token: Annotated[str, Depends(oauth2_scheme)]) -> dict:
        try:
            payload: dict = jwt_2.decode(token=token, key=AuthToken.SECRET_KEY, algorithms=[AuthToken.ALGORITHM])
